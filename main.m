...
state=zeros(number_of_states,1);
state(1)=4;
...
history_of_state=zeros(number_of_states,number_of_iterations);

model_simulation= @(state,input,ts) nominal_discrete_model(state,input,time_step);

nonlinear_constraint = @(state) safe_error(state,obstacles,max_y,max_theta);

for i=1:number_of_iterations
    input = nominal_mpc(state,model_simulation,Q,R,nominal_velocity,horizon,...
    nonlinear_constraint,[min_vel;-max_rot_vel],[max_vel;max_rot_vel]);
